/**
 * Agents-specific Supabase client for direct project connection
 * 
 * This client connects to the dedicated agents Supabase project (ghmmdochvlrnwbruyrqk)
 * allowing Claude Code to deploy agent-specific edge functions and migrations
 * without conflicting with Lovable Cloud auto-deployment.
 * 
 * Authentication uses JWT passthrough - Lovable auth tokens are verified on this project.
 */
import { createClient, SupabaseClient } from '@supabase/supabase-js';
import type { Database } from './types';

// Direct Supabase project for agents (deployed via Claude Code)
const AGENTS_SUPABASE_URL = import.meta.env.VITE_AGENTS_SUPABASE_URL || 'https://ghmmdochvlrnwbruyrqk.supabase.co';
const AGENTS_SUPABASE_KEY = import.meta.env.VITE_AGENTS_SUPABASE_KEY;

// Lazy-initialized client instance
let _supabaseAgents: SupabaseClient<Database> | null = null;

// Helper to check if agents client is properly configured
export const isAgentsClientConfigured = (): boolean => {
  return !!(AGENTS_SUPABASE_URL && AGENTS_SUPABASE_KEY);
};

// Get the agents client (lazy initialization)
// Returns null if not configured
export const getAgentsClient = (): SupabaseClient<Database> | null => {
  if (!isAgentsClientConfigured()) {
    console.warn('[agents-client] Not configured - VITE_AGENTS_SUPABASE_KEY is missing');
    return null;
  }
  
  if (!_supabaseAgents) {
    _supabaseAgents = createClient<Database>(
      AGENTS_SUPABASE_URL,
      AGENTS_SUPABASE_KEY!,
      {
        auth: {
          storage: localStorage,
          persistSession: true,
          autoRefreshToken: true,
        }
      }
    );
  }
  
  return _supabaseAgents;
};

// Legacy export for backward compatibility - throws helpful error if not configured
export const supabaseAgents = new Proxy({} as SupabaseClient<Database>, {
  get(target, prop) {
    const client = getAgentsClient();
    if (!client) {
      throw new Error(
        'Agents Supabase client is not configured. ' +
        'Set VITE_AGENTS_SUPABASE_KEY environment variable or use getAgentsClient() to check availability first.'
      );
    }
    return (client as any)[prop];
  }
});

// Get the configured agents project URL (for display/debugging)
export const getAgentsProjectUrl = (): string => AGENTS_SUPABASE_URL;
