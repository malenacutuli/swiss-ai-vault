...
function AvatarCore({ fft, isSpeaking, isListening }: AvatarCoreProps) {
  ...
  const primaryColor = useMemo(() => {
    if (isSpeaking) return '#2A8C86'; // Teal when speaking
    if (isListening) return '#3B82F6'; // Blue when listening
    return '#64748B'; // Slate when idle
  }, [isSpeaking, isListening]);

  const secondaryColor = useMemo(() => {
    if (isSpeaking) return '#10B981'; // Emerald
    if (isListening) return '#8B5CF6'; // Violet
    return '#94A3B8'; // Slate
  }, [isSpeaking, isListening]);
...
export function HealthAvatar3D({ fft, isSpeaking, isListening, className }: HealthAvatar3DProps) {
  return (
    <div className={className}>
      ...
      >
        ...
        <pointLight position={[-10, -10, -10]} intensity={0.5} color="#2A8C86" />
        <spotLight
          position={[0, 5, 5]}
          angle={0.3}
          penumbra={1}
