...
echo "=========================================="
echo "Phase 4 Deployment: Document Generation"
echo "=========================================="
echo ""
...
if [ -z "$API_SERVICE" ]; then
    ...
    echo ""
    echo "To test document generation, use port-forward:"
    echo "  kubectl port-forward -n agents svc/agent-api-service 8000:8000"
    echo "  curl http://localhost:8000/api/documents/formats"
else
    ...
    echo "Testing document generation - Supported formats..."
    curl -s "http://${API_SERVICE}:8000/api/documents/formats" | jq . || echo "Document API not yet accessible"
    echo ""
fi
...
echo "  ✓ Multi-Format Document Generation"
...
echo ""
echo "API Endpoints:"
echo "  POST   /api/documents/generate"
echo "  GET    /api/documents/download/{format}/{filename}"
echo "  GET    /api/documents/formats"
echo "  DELETE /api/documents/cleanup"
echo ""
echo "Test Document Generation:"
echo '  curl -X POST http://api.swissbrain.ai/api/documents/generate \'
echo '    -H "Content-Type: application/json" \'
echo '    -d '"'"'{'
echo '      "title": "Test Document",'
echo '      "format": "docx",'
echo '      "sections": [{"heading": "Test", "type": "text", "content": "Hello"}]'
echo '    }'"'"
echo ""
echo "Monitor with:"
