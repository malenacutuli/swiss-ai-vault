...
serve(async (req) => {
  ...
  console.log(`[gmail-oauth] ${req.method} ${path}`);
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      console.log(`[gmail-oauth] POST action: ${action}`);
      ...
      if (action === 'authorize') {
        ...
        // Generate state for CSRF protection (encode user_id in state)
        ...
        console.log(`[gmail-oauth] Generated auth URL for user ${user.id}`);
    ...
    if (path.endsWith('/callback')) {
      ...
      if (error) {
        ...
        return Response.redirect(`${FRONTEND_URL}/chat?error=gmail_auth_failed`, 302);
      }
      ...
      try {
        ...
        console.error('[gmail-oauth] Invalid state token');
      ...
      if (Date.now() - timestamp > 15 * 60 * 1000) {
        console.error('[gmail-oauth] State token expired');
      ...
      if (existing) {
        ...
        if (updateError) {
          console.error('[gmail-oauth] Failed to update integration:', updateError);
        ...
        if (insertError) {
          console.error('[gmail-oauth] Failed to insert integration:', insertError);
          return Response.redirect(`${FRONTEND_URL}/chat?error=db_error`, 302);
        }
      }

      console.log(`[gmail-oauth] Integration saved for user ${user_id}`);
      return Response.redirect(`${FRONTEND_URL}/chat?success=gmail`, 302);
    }
